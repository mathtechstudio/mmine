...
import 'skip_to_next_use_case_test.mocks.dart';
...
void main() {
  ...
  test('should return failure when skip fails', () async {
    when(
      mockRepository.skipToNext(),
    ).thenAnswer((_) async => Left(PlaybackFailure('No next track')));

    final result = await useCase(NoParams());

    expect(result, Left(PlaybackFailure('No next track')));
    verify(mockRepository.skipToNext());
    verifyNoMoreInteractions(mockRepository);
  });
}
